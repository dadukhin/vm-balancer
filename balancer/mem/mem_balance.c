...
char *getText(int tag)
{
        ...
        case VIR_DOMAIN_MEMORY_STAT_UNUSED:
                ret = "UNUSED"; //completely unused memory
                break;
        case VIR_DOMAIN_MEMORY_STAT_AVAILABLE:
                ret = "AVAILABLE"; //how much mem is available to the guest
                break;
        case VIR_DOMAIN_MEMORY_STAT_ACTUAL_BALLOON:
                ret = "TOTAL MEMORY"; //CURRENT_BALLOON
                break;
        case VIR_DOMAIN_MEMORY_STAT_RSS:
                ret = "RSS (Resident Set Size)";
                break;
        ...
        case VIR_DOMAIN_MEMORY_STAT_USABLE:
                ret = "FREE MEM";
                /*how much 'free' mem the guest currently has:
                   this is arguably more accurate since it an estimate of how much mem would
                   be available to new applications. stat_unused doesn't include cached stuff
        ...
        for(int i  = 0; i < numDoms; i++) {
                ...
                for (int x = 0; x < nr_stats; x++) {
                        ...
                        if (//tmpStats[x].tag == VIR_DOMAIN_MEMORY_STAT_UNUSED ||
                                ...
                                printf("---%s: %llu\n", getText(tmpStats[x].tag), tmpStats[x].val/1024);
                        }
...
//              virDomainSetMemoryFlags(doms[0], avail+THRESH_STARVE, flags);
                /*ALGO*/
//avail - reclaimed = new balloon
//in virt manager, current allocated mem = libvirt current balloon size


/*
for some reason, per the libvirt api, the balloon driver does not follow normal convention
instead of inflating on memory free, it will instead deflate!
*/
                ...
                for (int i = 0; i < numDoms; i++) {
                        ...
                        unsigned long long memFree = DOMAIN_MEM_INFO[VIR_DOMAIN_MEMORY_STAT_NR * i + 8];
                        //    printf("FREEEE %llu %llu\n", memFree, THRESH_STARVE);
